...
public class PacketLogger extends Module {
    public PacketLogger() {
        super(Categories.DEFAULT, "PacketLogger", "Logs packets or whatever you want. (only packets rn)");
    }
    ...
    @Nullable
    private String onCustomPayload(Packet<?> p) {
        ...
        customPayload.write(payloadBuffer);
    ...
    private String packetMessage(Packet<?> packet) {
        ...
        if (packet instanceof CustomPayloadC2SPacket p) return onCustomPayload(p);
        ...
        if (packet instanceof PlayerInteractEntityC2SPacket p) return "PlayerInteractEntity id:" + p.entityId;
        ...
        if (packet instanceof SpectatorTeleportC2SPacket p) return "SpectatorTeleport UUID:" + p.targetUuid;
        ...
        if (packet instanceof CustomPayloadS2CPacket p) return onCustomPayload(p);
        ...
        if (packet instanceof EntityS2CPacket p) return "Entity id:" + p.id + " deltaX:" + p.getDeltaX() + " deltaY:" + p.getDeltaY() + " deltaZ:" + p.getDeltaZ() + " yaw:" + p.getYaw() + " pitch:" + p.getPitch() + " hasRotation:" + p.hasRotation() + " posChange:" + p.isPositionChanged() + " ground:" + p.isOnGround();
        if (packet instanceof EntitySetHeadYawS2CPacket p) return "EntitySetHeadYaw id:" + p.entity;
        ...
        if (packet instanceof EntityStatusS2CPacket p) return "EntityStatus status:" + p.getStatus() + " id:" + p.id;
        ...
        if (packet instanceof ScoreboardScoreUpdateS2CPacket p) return "ScoreboardPlayerUpdate playerName:" + p.scoreHolderName() + " objectiveName:" + p.objectiveName() + " score:" + p.score();
        ...
        if (packet instanceof SetCameraEntityS2CPacket p) return "SetCameraEntity id:" + p.entityId;
