...
//shit stollen from limhax, sometimes flags, sometimes not (okay, it flags most of the times)
//also dont go forward when towering
...
public class VulcanScaffold extends Module {
    ...
    private final Setting<SettingColor> sideColor = sgRender.add(new ColorSetting.Builder()
        ...
        .defaultValue(new SettingColor(0, 236, 255, 10))
        .visible(render::get)
        .build()
    );
    ...
    private final Setting<SettingColor> lineColor = sgRender.add(new ColorSetting.Builder()
        ...
        .defaultValue(new SettingColor(6, 242, 255))
        .visible(render::get)
        .build()
    );
    ...
    public VulcanScaffold() {
        super(Categories.DEFAULT, "Vulcant't scaffold", "This shit is made by limhax, im not responsible for it, also it sometimes flags sometimes not!");
    }
    ...
    private void onTick(PlayerMoveEvent event) {
        ...
        if(mc.player.age % 4 == 0 && mc.options.jumpKey.isPressed()) {
            mc.player.jump();
        }

        if(mc.player.age % 19 == 0) {
            ...
            for (int x = (int) (mc.player.getX() - placeRange.get()); x < mc.player.getX() + placeRange.get(); x++) {
                for (int z = (int) (mc.player.getZ() - placeRange.get()); z < mc.player.getZ() + placeRange.get(); z++) {
                    for (int y = (int) Math.max(mc.world.getBottomY(), mc.player.getY() - placeRange.get()); y < Math.min(mc.world.getTopY(), mc.player.getY() + placeRange.get()); y++) {
                        ...
                        if (mc.player.getEyePos().squaredDistanceTo(Vec3d.ofCenter(bp.offset(BlockUtils.getDirection(bp)))) > 36) continue;
    ...
    private boolean place(BlockPos bp) {
        ...
        if (mc.player.isOnGround() || !mc.player.isOnGround()) {
            BlockUtils.place(bp, item, rotate.get(), 1, renderSwing.get(), true);

            switch (count) {
                case 0:
                    Rotations.rotate(mc.player.getYaw()-180, 70);
            ...
            if(mc.player.age % 2 == 0 && mc.player.isOnGround()) {
